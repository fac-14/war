...
global.fetch = jest.fn()
    .mockImplementation(url => {
        ...
        return Promise.resolve({ status: 200, json: () => Promise.resolve(mockResponse) })
    });

afterEach(cleanup);

describe('Board', () => {
    const { container, debug, getByText, getByLabelText, getByTestId } = render(
        <Board />
    );
    const button = getByText("Play!");
    it('renders the button', () => {
        expect(button.id).toEqual('play');
    });
})