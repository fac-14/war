import createDeck from './getData';

const mockResponseId = '54wuev6mlxyr';

const mockResponseDeck = `
[
{
    code: "AS",
        value: "ACE",
...
global.fetch = jest.fn()
    .mockImplementation(url => {
        let mockResponse
        if (url === 'https://deckofcardsapi.com/api/deck/new/shuffle/?deck_count=1') {
            mockResponse = mockResponseId;
        }
        else {
            mockResponse = mockResponseDeck;
        }
        return Promise.resolve({ text: () => Promise.resolve(mockResponse) })
    })

test('test if createDeck gets cardDeck', () => {
    createDeck().then(twoPiles =>
        expect(twoPiles.length).toEqual(2))
})
